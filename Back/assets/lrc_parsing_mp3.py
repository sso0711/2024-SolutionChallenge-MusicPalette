...
from lyrics_naver import get_lyrics_naver
...
success = 0
fail = 0
lrc_directory = './assets/uploads/lyrics'
lrc_filename = ''
lrc_filepath = ''

music_directory = './assets/uploads/musics/'
music_filepath = []
...
try:
    if len(FILE_LIST) != 0:  # mp3 곡이 있는지 확인하기
        ...
        if i == len(available_file)-1:
            ...
            if fail==1:
                # " (" 문자열을 기준으로 분리
                split_position = realFileName.find(" (")  # " (" 위치를 찾기
                if split_position != -1:  # " (" 가 문자열에 있으면
                    split_title = realFileName[:split_position]  # 그 위치까지의 부분 추출
                else:
